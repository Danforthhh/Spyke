...
export default function ProductPicker({ products, favoriteId, onSelect, onSetFavorite, onAddProduct, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-lg max-h-[80vh] flex flex-col shadow-xl dark:shadow-slate-900/60"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center px-5 py-3.5 border-b border-slate-100 dark:border-slate-800">
          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Your product</span>
          <button
            onClick={onClose}
            className="ml-auto text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 text-lg leading-none cursor-pointer bg-transparent border-0 w-7 h-7 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >×</button>
        </div>
        ...
        <div className="overflow-y-auto flex-1">
          {filtered.length === 0 && (
            <div className="py-10 text-center text-sm text-slate-400 dark:text-slate-500">No products found.</div>
          )}
          {filtered.map(p => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-3 px-5 py-3 border-b border-slate-50 dark:border-slate-800/60 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
              >
                <div className="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-900 flex items-center justify-center text-[11px] font-bold text-indigo-500 dark:text-indigo-400 flex-shrink-0">
                  {initials}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-800 dark:text-slate-200 truncate">{p.name}</div>
                  <div className="text-[11px] text-slate-400 dark:text-slate-500 truncate">{p.category}</div>
                </div>
                <button
                  ...
                  className={`text-base p-1 rounded-md transition-colors cursor-pointer bg-transparent border-0 ${
                    isFav ? 'text-amber-400 hover:text-amber-500' : 'text-slate-200 dark:text-slate-700 hover:text-slate-400 dark:hover:text-slate-500'
                  }`}
                >
                  {isFav ? '★' : '☆'}
                </button>
        ...
        <div className="border-t border-slate-100 dark:border-slate-800">
          {!showAddForm ? (
            <button
              ...
              className="w-full px-5 py-3 text-left text-sm text-indigo-500 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors cursor-pointer bg-transparent border-0 flex items-center gap-1.5 font-medium"
            >
              <span className="text-base leading-none">+</span> Add a product
            </button>
          ) : (
            <div className="px-5 py-4 space-y-3">
              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">New product</p>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Name *</div>
                  <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="ProTop" className={inputCls} />
                </div>
                <div>
                  <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Category *</div>
                  <input value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder="B2B SaaS CRM" className={inputCls} />
                </div>
              </div>
              <div>
                <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Tagline</div>
                <input value={newTagline} onChange={e => setNewTagline(e.target.value)} placeholder="One-liner…" className={inputCls} />
              </div>
              <div>
                <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Positioning</div>
              ...
              <div>
                <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Features (one per line)</div>
              ...
              <div className="flex gap-2 pb-1">
                <button
                  ...
                  className={`text-sm font-semibold px-4 py-2 rounded-lg transition-colors ${
                    saving ? 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-600 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700 cursor-pointer'
                  }`}
                >
                ...
                <button
                  ...
                  className="text-sm px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600 cursor-pointer"
                >
                  Cancel
                </button>
              </div>
